...
const Hero = () => {
  const badge = "Meet the Averto";
  const description =
    "Stop recreating whole workflows of cutom rags and pay to ai developers , instead use averto to get started";
      ...
      <Wrapper className="relative z-10">
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="text-4xl md:text-6xl font-semibold tracking-tight font-heading mt-8">
            <Balancer>
              {"Deploy AI Agents".split(" ").map((word, index) => (
                <motion.span
              ...
              <br />
              {"That Works 24/7".split(" ").map((word, index) => (
                <motion.span
                  initial={{ filter: "blur(10px)", opacity: 0, y: 10 }}
                  animate={{ filter: "blur(0px)", opacity: 1, y: 0 }}
