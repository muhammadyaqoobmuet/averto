...
export default function GapTab({ chatbotId }: GapTabProps) {
  ...
  const fetchMissedQueries = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/chatbots/${chatbotId}/missed-queries`,
        {
  ...
  // Filter to confidence between 25% and 35% (low confidence zone)
  const gaps = useMemo(
    () => allMissed.filter((q) => q.topScore >= 0.25 && q.topScore < 0.35),
    [allMissed],
  );
  ...
  const avgScore =
    totalGaps > 0
      ? Math.round(
          gaps.reduce((sum, q) => sum + q.topScore * 100, 0) / totalGaps,
        )
      : 0;
  ...
  const sortedGaps = useMemo(() => {
    return [...gaps].sort((a, b) => {
      ...
      return (
        dir * (new Date(a.askedAt).getTime() - new Date(b.askedAt).getTime())
      );
    });
  ...
  const handleDownload = async () => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      const res = await fetch(
        `${API_URL}/api/chatbots/${chatbotId}/gap-report`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
    ...
    <div className="max-w-3xl space-y-5 animate-fade-up">
      ...
      <div className="flex items-start gap-2.5 p-3.5 rounded-xl border border-blue-500/20 bg-blue-500/5">
        ...
        <p className="text-[12px] text-blue-400 leading-relaxed">
          Gaps are queries where the chatbot had between 25% and 35% confidence
          (low confidence zone). Review these to improve your knowledge base.
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="p-4 rounded-xl border border-[var(--border)] bg-[var(--surface)]">
          ...
          <p className="text-[22px] font-bold text-[var(--danger)] tabular-nums">
            {totalGaps}
          </p>
        </div>
        <div className="p-4 rounded-xl border border-[var(--border)] bg-[var(--surface)]">
          ...
          {topKeywords.length > 0 ? (
            <div className="flex flex-wrap gap-1 mt-1">
              {topKeywords.map((kw) => (
                <span
                  key={kw}
                  className="px-1.5 py-0.5 rounded text-[10px] bg-[var(--bg)] border border-[var(--border)] text-[var(--text-secondary)]"
                >
                  {kw}
                </span>
              ))}
          ...
          <div className="divide-y divide-[var(--border)]">
            {sortedGaps.map((q) => {
              ...
              return (
                ...
                >
                  <p className="text-[13px] text-[var(--text)] truncate pr-2">
                    {q.query}
                  </p>
                  <span
                    className={`inline-flex items-center px-2 py-0.5 rounded-full border text-[11px] font-semibold tabular-nums ${badgeColor}`}
                  >
                    {pct}%
                  </span>
