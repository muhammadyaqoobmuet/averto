...
export default function ModelSettings({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (usePlatformKeys) {
        toast({ type: "success", title: "Using platform keys" });
      } else {
        toast({
          type: "success",
          title: "API key saved",
          description: "Your custom model settings are active.",
        });
      }
      onSave();
    } catch {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast({
          ...
          title: "Clear failed",
          description:
            (data as { error?: string }).error || "Failed to clear custom key.",
        });
        return;
      }
      setUsePlatformKeys(true);
      setApiKey("");
      toast({ type: "success", title: "Reverted to platform keys" });
      onSave();
    } catch {
      toast({
        type: "error",
  ...
  const selectedOption =
    MODEL_OPTIONS.find((m) => m.value === selectedModel) ?? MODEL_OPTIONS[0];
          ...
          <div className="space-y-4">
              ...
              <div className="relative">
                <input
                  ...
                  placeholder={
                    hasCustomKey
                      ? "••••••••••••  (enter to replace)"
                      : `Enter your ${selectedOption.provider} API key`
                  }
            ...
            <div className="flex items-start gap-2 px-3 py-2.5 rounded-lg border border-[var(--border)] bg-[var(--bg)]">
              ...
              <p className="text-[11px] text-[var(--text-muted)] leading-relaxed">
                Your API key is stored encrypted. We use it only for this
                chatbot&apos;s responses.
              </p>
            </div>
          </div>
