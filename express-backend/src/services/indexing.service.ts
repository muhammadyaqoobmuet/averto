...
export async function indexPageContent(
  ...
  const splitter = isMarkdown
    ? new MarkdownTextSplitter({ chunkSize: 1200, chunkOverlap: 120 })
    : new RecursiveCharacterTextSplitter({
        chunkSize: 1200,
        chunkOverlap: 120,
      });

  let docs = await splitter.createDocuments([content], [{ chatbotId, pageId }]);
  docs = docs.filter((d) => d.pageContent.trim().length > 10);
