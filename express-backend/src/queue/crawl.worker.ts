 ...
 */
export const crawlWorker = new Worker('crawl-queue', async (job: Job) => {
    const { chatbotId, url } = job.data;

    try {
        logger.info(`[CrawlWorker] Starting job for Bot ${chatbotId} URL: ${url}`);
        ...
        if (!pages || pages.length === 0) {
            throw new Error('Python service returned no pages. This might be due to robot.txt blocks or connection issues.');
        }
        ...
        for (const page of pages) {
            ...
            if (!page.markdown || page.markdown.trim().length < 50) {
                logger.warn(`[CrawlWorker] Skipping empty or low-quality page: ${page.url}`);
                continue;
            }
            ...
            if (indexed === 0) {
                ...
                logger.warn(`[CrawlWorker] No vectors generated for ${page.url}, record removed.`);
                continue;
            }

            totalChunks += indexed;
            pagesIndexed++;
            logger.info(`[CrawlWorker] Page ${page.url} indexed with ${indexed} chunks`);
        }

        // 6. Handle total failure (site crawled but nothing could be indexed)
        if (totalChunks === 0) {
            throw new Error('Crawl completed but no content could be indexed. Site may be empty or incompatible.');
        }
        ...
        logger.info(`[CrawlWorker] SUCCESS: Bot ${chatbotId}. ${totalChunks} vectors from ${pagesIndexed} pages.`);
